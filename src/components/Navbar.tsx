...
export const Navbar: React.FC = () => {
  ...
  return (
    <header 
      className="sticky top-0 z-40 w-full glass-panel border-b transition-all shadow-sm backdrop-blur-2xl pt-[calc(0.35rem+env(safe-area-inset-top,0px))] pb-2 px-2.5 sm:px-4 lg:px-8"
      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-1.5 sm:gap-4 w-full">
        ...
        <div className="flex items-center shrink-0 min-w-0">
          <button
            ...
            className="group flex items-center gap-2 p-1 rounded-2xl hover:bg-slate-800/40 transition-all text-start shrink-0 active:scale-95 border border-transparent hover:border-slate-700/40"
            title={isAr ? 'تعديل الملف الشخصي والبيانات' : 'Edit Profile'}
          >
            {/* Avatar */}
            <div className="relative shrink-0">
              <div className="w-9 h-9 sm:w-11 sm:h-11 rounded-2xl bg-gradient-to-tr from-indigo-500 via-purple-500 to-pink-500 p-0.5 shadow-md shadow-indigo-500/20 group-hover:scale-105 transition-transform">
                <div 
                ...
                >
                  {profile.avatarType === 'upload' && profile.avatarValue ? (
                    ...
                    <span className="text-base sm:text-xl leading-none">{profile.avatarValue || '🎓'}</span>
                  )}
                </div>
              </div>

              {/* Mini Level badge */}
              <div className="absolute -bottom-1 -right-1 bg-amber-500 text-slate-950 font-black text-[8px] sm:text-[9px] px-1 rounded-md shadow-sm border border-slate-950 leading-tight">
                L{currentLevel}
              </div>
            </div>

            {/* Name & Stage info */}
            <div className="flex flex-col justify-center min-w-0 max-w-[100px] min-[360px]:max-w-[125px] sm:max-w-[170px] md:max-w-[210px]">
              <div className="flex items-center gap-1">
                <span 
                  className="font-black text-xs sm:text-sm leading-tight truncate"
              ...
              <div className="flex items-center gap-1 mt-0.5">
                ...
                {(profile.streakDays || 1) > 0 && (
                  <span 
                    className="hidden min-[420px]:inline-flex items-center gap-0.5 text-[9px] font-black px-1.5 py-0.2 rounded-full bg-amber-500/15 border border-amber-500/30 text-amber-400 shrink-0"
                    title={isAr ? `تتابع دراسي: ${profile.streakDays || 1} يوم` : `${profile.streakDays || 1} day streak`}
                  >
        ...
        {/* CENTER: Desktop Navigation Tabs (Hidden on mobile < lg) */}
        <nav 
        ...
        {/* RIGHT: Action Buttons (Responsive and clean) */}
        <div className="flex items-center gap-1 sm:gap-1.5 shrink-0">
          
          {/* INSTALL APP PROMINENT CTA */}
          <button
            onClick={handleInstallClick}
            className={`relative overflow-hidden group flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1.5 sm:py-2 rounded-xl text-[11px] sm:text-xs font-black transition-all shadow-md active:scale-95 cursor-pointer shrink-0 ${
              isAppInstalled
            ...
            title={isAppInstalled ? (isAr ? 'التطبيق مثبّت 📱' : 'App Installed 📱') : (isAr ? 'تثبيت التطبيق على الشاشة الرئيسية 📲' : 'Install Yasta Zakir 📲')}
          >
            {isAppInstalled ? (
            ...
            <span className="leading-none">
              {isAppInstalled 
                ...
                : (
                  <>
                    <span className="hidden min-[480px]:inline">{t('navInstallApp')}</span>
                    <span className="min-[480px]:hidden">{isAr ? 'تثبيت' : 'Install'}</span>
          ...
          {/* Athkar & Tasbeeh Button (Visible md+) */}
          <button
            onClick={() => {
              haptic.light();
              setIsAthkarModalOpen(true);
            }}
            className="hidden sm:flex items-center gap-1 px-2.5 py-1.5 sm:py-2 rounded-xl border bg-emerald-500/10 border-emerald-500/30 text-emerald-400 hover:text-white hover:bg-emerald-600/30 transition-all text-xs font-black active:scale-95"
          ...
          {/* AI Study Planner (Visible sm+) */}
          <button
            onClick={() => {
              haptic.light();
              setIsAIPlannerModalOpen(true);
            }}
            className="p-1.5 sm:px-2.5 sm:py-2 rounded-xl border bg-gradient-to-r from-purple-600/25 via-indigo-600/25 to-pink-600/25 border-purple-500/40 text-purple-300 hover:text-white transition-all text-xs font-black flex items-center gap-1 active:scale-95"
          ...
          <div className="relative" ref={moreMenuRef}>
            <button
              ...
              title={isAr ? 'المزيد من الأدوات والإعدادات ⚡' : 'More Options ⚡'}
            >
            ...
            <AnimatePresence>
              {isMoreMenuOpen && (
                <motion.div
                  ...
                  className={`absolute top-full mt-2 ${isAr ? 'left-0' : 'right-0'} w-60 rounded-2xl p-1.5 shadow-2xl border z-50 backdrop-blur-3xl`}
                  style={{
                ...
                >
                  ...
                  <button
                    ...
                    className="sm:hidden w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-bold hover:bg-emerald-600/20 text-emerald-400 text-start transition-colors"
                  >
                    <Heart className="w-4 h-4 text-emerald-400 shrink-0" />
                    <span>{t('navAthkar')}</span>
                  </button>
                  ...
                  <button
                    ...
                    className="w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-bold hover:bg-purple-600/20 text-start transition-colors"
                  >
                    <Volume2 className="w-4 h-4 text-purple-400 shrink-0" />
                    <span>{isAr ? 'الأصوات والاهتزازات والمظهر 🎵' : 'Audio, Haptics & Themes 🎵'}</span>
                  </button>
